/**
 * 聊天消息列表：ScrollArea + map 分组渲染。
 *
 * 从 dashboard routes/chat/MessageList.tsx 搬移（290 行——ScrollArea + map 未虚拟化，
 * 2026-08-09 CC 决策 A 确认：聊天会话消息量场景 ScrollArea 足够）。
 * 行为等价：分组/滚动锚点/scrollToMessage 高亮/空态欢迎页/语音播放行常驻。
 */
...
import { ChatScrollContext, type ChatScrollContextValue } from './chat-scroll-context'
  ...
  return (
    <div className="min-w-0 min-h-0 flex-1 overflow-hidden">
      <ScrollArea
        ...
        viewportClassName="[&>div]:!block [&>div]:!min-w-0 [&>div]:w-full"
      >
        <ChatScrollContext value={scrollContextValue}>
