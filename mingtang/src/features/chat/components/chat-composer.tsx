/**
 * 聊天输入区：自适应高度 Textarea + 发送按钮 + 图片预览条 + 表情按钮。
 *
 * 从 dashboard routes/chat/ChatComposer.tsx 搬移（139 行——行为等价）。
 * Enter 发送（isComposing 保护中文输入法）+ 图片上限 8 张 + 未连接态禁用。
 */
...
export function ChatComposer({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    // Enter 发送（isComposing 保护中文输入法——Shift+Enter 换行）
