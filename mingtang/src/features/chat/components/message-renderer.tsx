/**
 * 消息段渲染器：12 种段类型 switch。
 *
 * 从 dashboard routes/chat/MessageRenderer.tsx 搬移适配（282 行——行为等价）。
 * 适配点：useToast → sonner toast() 调用（CC 底座决策 #4）。
 */
...
import { useChatScroll } from './chat-scroll-context'
import type {
  AtSegmentData,
  ChatMessage,
  MessageSegment,
...
function ReplySegmentBlock({ data }: { data: ReplySegmentData }) {
  ...
  const handleClick = () => {
    ...
    if (!found) {
      // sonner 适配：原版 useToast variant: 'destructive' → toast.error
      toast.error(
        t('chat.toast.replyNotFoundTitle', { defaultValue: '原始消息不在当前视图' }),
        {
