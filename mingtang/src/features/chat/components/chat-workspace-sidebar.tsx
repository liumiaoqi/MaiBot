/**
 * 桌面会话侧栏（含新建虚拟会话入口——孤儿补全 ADR-6）。
 *
 * 从 dashboard routes/chat/ChatWorkspaceSidebar.tsx 搬移适配（269 行——行为等价）。
 * 桌面端会话列表 + 用户身份卡（内联编辑昵称）+ 新建虚拟会话入口按钮。
 *
 * 孤儿补全（ADR-6）：原版 VirtualIdentityDialog 组件完整但无入口——
 * 本组件的"新建虚拟会话"按钮（onAddVirtual）触发 VirtualIdentityDialog，
 * 由父组件 ChatPage 负责弹窗 open 状态与 person-api 数据加载。
 */
 ...
 * 桌面会话侧栏。
 *
 * 三段式布局：头部（标题 + 新建按钮）/ 会话列表（ScrollArea）/ 底部（用户身份卡）。
 */
