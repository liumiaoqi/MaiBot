...
export function usePluginList() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- loadPlugins 是 async 函数，setState 在 await 后的异步回调中；首次加载 loading 初值已为 true，setLoading(true) 为 no-op
    loadPlugins()
  ...
  const performTogglePlugin = async (plugin: InstalledPlugin) => {
    ...
    try {
      ...
      toast.success(toggleResult.enabled ? '插件已启动' : '插件已关闭', {
