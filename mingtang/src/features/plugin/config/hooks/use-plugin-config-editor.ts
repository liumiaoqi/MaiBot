...
export function usePluginConfigEditor(options: UsePluginConfigEditorOptions) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- loadConfig 是 async 函数，setState 在 await 后的异步回调中；首次加载 loading 初值已为 true，setLoading(true) 为 no-op
    loadConfig()
  }, [loadConfig])
  ...
  }, [config, editMode, plugin.id, sourceCode])
