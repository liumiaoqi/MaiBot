...
import type { FlowEdge, FlowNode, FlowNodeData, GraphEdge, GraphNode } from '../../types/graph-types'
...
export function GraphVisualization({ graphData, onNodeClick, onEdgeClick, loading = false }: GraphVisualizationProps) {
  ...
  return (
    ...
    >
      {/* 加载态用覆盖层——不卸载 ReactFlow 实例（CX 审核：卸载重挂丢视口/缩放 + 重播 fitView） */}
      {loading && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-background/60" aria-label="图谱加载中">
