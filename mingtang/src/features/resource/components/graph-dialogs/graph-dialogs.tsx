...
export function NodeDetailDialog({
  ...
  // open 关闭 → 重置复选框——渲染期调整模式（审核修复：rAF 规避）
  const [prevNodeOpen, setPrevNodeOpen] = useState(open)
  if (!open && prevNodeOpen) {
    setPrevNodeOpen(open)
...
export function EdgeDetailDialog({
  ...
  // open 关闭 → 重置复选框——渲染期调整模式（审核修复：rAF 规避）
  const [prevEdgeOpen, setPrevEdgeOpen] = useState(open)
  if (!open && prevEdgeOpen) {
    setPrevEdgeOpen(open)
    setIncludeParagraphs(false)
  }
...
export function RelationDetailDialog({
  ...
  // open 关闭 → 重置复选框——渲染期调整模式（审核修复：rAF 规避）
  const [prevRelationOpen, setPrevRelationOpen] = useState(open)
  if (!open && prevRelationOpen) {
    setPrevRelationOpen(open)
    setIncludeParagraphs(false)
  }
