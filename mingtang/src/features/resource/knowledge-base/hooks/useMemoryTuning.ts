...
export function useMemoryTuning({ active, onRuntimeChanged }: UseMemoryTuningOptions): UseMemoryTuningResult {
  ...
  const submitTuningTask = useCallback(async () => {
    try {
      ...
      toast.success('新的检索调优任务已经进入队列')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : '未知错误')
    } finally {
      setCreatingTuning(false)
    }
  }, [tasksQuery, tuningIntensity, tuningObjective, tuningSampleSize, tuningTopKEval])
  ...
  const applyBestTask = useCallback(
    async (taskId: string) => {
      try {
        ...
        toast.success(result.persisted ? `任务 ${taskId} 的最佳轮次已经写入运行时和配置文件` : `任务 ${taskId} 的最佳轮次已经写入运行时`)
      } catch (error) {
        toast.error(error instanceof Error ? error.message : '未知错误')
      }
    },
    [onRuntimeChanged, persistBestProfile, profileQuery, tasksQuery],
  )
