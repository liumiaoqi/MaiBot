...
export function useImportQueue({ active, buildRetryOverrides }: UseImportQueueOptions): UseImportQueueResult {
  ...
  // 任务列表查询失败时派生错误文案（与本地异步操作错误合并后呈现）
  const tasksError = tasksQuery.error
  const tasksErrorText = tasksError
  ...
  const loadImportTaskDetail = useCallback(
    ...
    [importChunkOffset, loadImportChunks, selectedImportFileId],
  )
  ...
  const cancelSelectedImportTask = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`任务 ${selectedImportTaskId.slice(0, 12)} 正在取消`)
    } catch (error) {
      const message = error instanceof Error ? error.message : '取消导入任务失败'
      setImportErrorText(message)
      toast.error(message)
    }
  }, [loadImportTaskDetail, refreshImportQueue, selectedImportTaskId])
  ...
  const retrySelectedImportTask = useCallback(async () => {
    ...
    try {
      ...
      toast.success(nextTaskId ? `重试任务 ${nextTaskId.slice(0, 12)} 已进入队列` : '失败项已提交重试')
    } catch (error) {
      const message = error instanceof Error ? error.message : '重试失败项失败'
      setImportErrorText(message)
      toast.error(message)
    }
  }, [buildRetryOverrides, loadImportTaskDetail, refreshImportQueue, selectedImportTaskId])

  // 任务加载后若未选中任务则自动选第一个；选中任务被删除则切到第一个（沿用原 effect 6-7 语义）
  // 渲染期 setState：未选中任务且列表非空时自动选第一个
  if (active && !selectedImportTaskId && importTasks.length > 0) {
    void selectImportTask(importTasks[0].task_id)
  }

  // 渲染期 setState：选中任务为空时清空衍生状态
  if (active && !selectedImportTaskId) {
    if (selectedImportTask !== null) {
      setSelectedImportTask(null)
    }
    if (selectedImportFileId !== '') {
      setSelectedImportFileId('')
    }
    if (importChunksPayload !== null) {
      setImportChunksPayload(null)
    }
  }

  // 渲染期 setState：选中任务不在列表中且列表非空时切到第一个
  if (
    active &&
  ...
  }, [active, importTasks, loadImportTaskDetail, selectedImportTaskId])
  ...
  return {
    ...
    importPollInterval,
    importErrorText: tasksErrorText || importErrorText,
