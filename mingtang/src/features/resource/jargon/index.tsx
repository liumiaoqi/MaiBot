/** R4-1-2-6: 从 dashboard 行为等价搬移——适配 useToast → sonner toast + ChatScopeFilterPanel 路径 */
import { useMutation, useQuery } from '@tanstack/react-query'
import { Check, Plus, Search, Trash2, X } from 'lucide-react'
import { useState } from 'react'

import { ChatScopeFilterPanel } from '@/components/biz/chat-scope-filter-panel'
...
export function JargonManagementPage() {
  ...
  const handleViewDetail = async (jargon: Jargon) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : '无法加载黑话详情')
    }
  }
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_data, jargon) => {
      toast.success(`已删除黑话: ${jargon.content}`)
  ...
  const batchDeleteMutation = useMutation({
    ...
    onSuccess: (_data, ids) => {
      toast.success(`已删除 ${ids.length} 个黑话`)
      list.clearSelection()
  ...
  const batchSetJargonMutation = useMutation({
    ...
    onSuccess: (_data, vars) => {
      toast.success(`已将 ${vars.ids.length} 个词条设为${vars.isJargon ? '黑话' : '无黑话'}`)
      list.clearSelection()
      invalidateJargon()
    },
  })
  ...
  const handleJumpToPage = (jumpToPage: string) => {
    ...
    if (targetPage >= 1 && targetPage <= list.totalPages) {
      ...
      toast.error(`请输入1-${list.totalPages}之间的页码`)
    }
  }
