import { useCallback, useContext, useEffect, useRef, useState, type FormEvent, type ReactElement } from 'react'
...
/** 齿轮 SVG 路径纯几何计算——生成单个齿轮的 path d 属性 */
function createGearPath(teeth: number, outerRadius: number, rootRadius: number): string {
...
/** future-retro 主题下的齿轮装饰 SVG */
function AuthRetroGears(): ReactElement {
  return (
...
export function AuthPage(): ReactElement {
  ...
  const { resolvedTheme, setTheme, dashboardStyle } = useContext(ThemeProviderContext)
  const showRetroGears = dashboardStyle === 'future-retro'
  ...
  // 主题切换（resolvedTheme 已解析 system 为实际值）
  const toggleTheme = () => {
  ...
  // 如果已经认证，直接跳转首页；否则尝试用 URL 中的 token 自动登录
  useEffect(() => {
    ...
    <div data-auth-page="true" className="relative isolate flex min-h-screen items-center justify-center overflow-hidden bg-background p-4">
      ...
      <Card className="relative z-10 w-full max-w-md shadow-2xl backdrop-blur-xl bg-card/80 border-border/50">
        ...
        <button
          ...
          title={resolvedTheme === 'dark' ? t('auth.switchToLight') : t('auth.switchToDark')}
        >
          {resolvedTheme === 'dark' ? (
