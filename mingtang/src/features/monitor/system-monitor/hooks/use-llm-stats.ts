/**
 * useLLMStats LLM 统计 hook（T1-6-4 搬移——React 19 适配）
 *
 * React 19 适配（design.md §2.1.3 D）：
 * - 模式 4：ws 订阅/退订在 effect 内做（useEffect 仅副作用不 setState）
 * - 模式 5：effect 内不直接调含同步 setState 的函数（fetchAllData 内 setIsLoading(true)）
...
export function useLLMStats(initialHours = DEFAULT_HOURS) {
  ...
  useEffect(() => {
    ...
    // 模式 5：首次加载含同步 setState（setIsLoading），用 setTimeout(0) 调度
