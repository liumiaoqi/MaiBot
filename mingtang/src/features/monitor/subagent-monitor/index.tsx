/**
 * SubAgentMonitorPage 子智能体监控页（T1-5-3 搬移——cockpit 只读模式）
 *
 * 定位：子智能体执行记录统计（REST only——零写操作）
 * 数据流：getAgentList + getSubAgentStats + getSubAgentRecords
 * 只读约束：零写操作（spec.md §4.3 #2）
 * React 19 适配（design.md §2.1.3 D）：filterAgent seed 用渲染期 setState（替代 effect 内 setState）
 * i18n：页面标题用 monitor.subagent.title（design.md §2.5），面板内文案保留源中文
 */
...
import { getAgentList, getSubAgentRecords, getSubAgentStats } from '@/lib/agent-api'
...
export function SubAgentMonitorPage() {
  ...
  return (
    <PageShell
      title={t('monitor.subagent.title')}
      actions={
        <Button
          variant="ghost"
          size="icon"
