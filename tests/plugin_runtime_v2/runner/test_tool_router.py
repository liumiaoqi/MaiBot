...
class TestExecuteSuccess:
    ...
    @pytest.mark.asyncio
    async def test_execute_returns_result(self, router, plugin):
        async def handler(plg, args):
            return {"ok": True}
        router.register("ok", plugin, handler)
        resp = await router.execute("ok", {})
        assert resp.success
        assert json.loads(resp.result) == {"ok": True}

    @pytest.mark.asyncio
    async def test_execute_sync_handler(self, router, plugin):
        def handler(plg, args):
            return {"sync": True}
        router.register("sync", plugin, handler)
        resp = await router.execute("sync", {})
        assert resp.success
...
class TestExecuteErrors:
    ...
    @pytest.mark.asyncio
    async def test_parameter_validation_failed(self, router, plugin):
        ...
        resp = await router.execute("validated", {})
        assert not resp.success
