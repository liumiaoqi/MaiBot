...
export async function installPlugin(pluginId: string, repositoryUrl: string, branch: string = 'main'): Promise<ApiResponse<{ success: boolean; message: string }>> {
  return toApiResponse(() =>
    backendApi.post<{ success: boolean; message: string }>('/api/webui/plugins/install', {
      body: {
        plugin_id: pluginId,
...
export async function uninstallPlugin(pluginId: string): Promise<ApiResponse<{ success: boolean; message: string }>> {
  return toApiResponse(() =>
    backendApi.post<{ success: boolean; message: string }>('/api/webui/plugins/uninstall', {
      body: {
        plugin_id: pluginId
      },
      errorMessage: '卸载插件失败',
...
export async function updatePlugin(pluginId: string, repositoryUrl: string, branch: string = 'main'): Promise<ApiResponse<UpdatePluginResult>> {
  return toApiResponse(() =>
    backendApi.post<UpdatePluginResult>('/api/webui/plugins/update', {
      body: {
        plugin_id: pluginId,
        repository_url: repositoryUrl,
        branch: branch
