...
class SDKMemoryKernel:
    ...
    async def initialize(self) -> None:
        ...
        self._admin_handlers = {
            ...
            "source": SourceAdminHandler(self),
        }

        from .services.hit_filter import HitFilterService
        self._hit_filter_service = HitFilterService(
            metadata_store=self.metadata_store,
            cfg=self._cfg,
        ...
        self._fuzzy_modify_service = FuzzyModifyService(
            ...
            profile_evidence_admin=lambda *a, **kw: self._profile_evidence_service.profile_evidence_admin(*a, **kw),
        ...
        self._feedback_correction_service = FeedbackCorrectionService(
            ...
            apply_v5_relation_action=lambda *a, **kw: self._v5_memory_service.apply_v5_relation_action(*a, **kw),
            ingest_text=self.ingest_text,
            refresh_person_profile=self.refresh_person_profile,
        ...
        self._search_service = SearchService(
            hit_filter_service=self._hit_filter_service,
            get_retriever=lambda: self.retriever,
            get_episode_retriever=lambda: self.episode_retriever,
            get_aggregate_query_service=lambda: self.aggregate_query_service,
            get_threshold_filter=lambda: self.threshold_filter,
            build_runtime_config=self._build_runtime_config,
        ...
        from .services.vector_rebuild import VectorRebuildService
        self._vector_rebuild_service = VectorRebuildService(
            vector_pool_manager=self._vector_pool_manager,
            embedding_manager_getter=lambda: self.embedding_manager,
            metadata_store_getter=lambda: self.metadata_store,
            vector_store_getter=lambda: self.vector_store,
            paragraph_vector_store_getter=lambda: self.paragraph_vector_store,
            graph_vector_store_getter=lambda: self.graph_vector_store,
            relation_vectors_enabled_getter=lambda: self.relation_vectors_enabled,
            embedding_dimension_getter=lambda: self.embedding_dimension,
            embedding_dimension_setter=lambda v: setattr(self, 'embedding_dimension', v),
            cfg=self._cfg,
            active_row_filter_sql=self._active_row_filter_sql,
            count_vector_rebuild_targets=self._count_vector_rebuild_targets,
            refresh_relation_write_service=self._refresh_relation_write_service,
            set_embedding_degraded=self._set_embedding_degraded,
            refresh_runtime_self_check=self._refresh_runtime_self_check,
            apply_self_check_dimension_result=self._apply_self_check_dimension_result,
            refresh_runtime_dependents=self._refresh_runtime_dependents,
            apply_runtime_sparse_mode=self._apply_runtime_sparse_mode,
            build_runtime_config=self._build_runtime_config,
            persist=self._persist,
            reload_dual_vector_stores_from_disk=self._reload_dual_vector_stores_from_disk,
            write_dual_vector_ready_manifest=self._write_dual_vector_ready_manifest,
            clear_legacy_single_vector_files_after_dual_ready=self._clear_legacy_single_vector_files_after_dual_ready,
            backfill_missing_dual_vector_pool_entries=self._backfill_missing_dual_vector_pool_entries,
            update_dual_vector_auto_migration_stage=self._update_dual_vector_auto_migration_stage,
            vector_rebuild_status_getter=self._vector_rebuild_status,
            vector_persist_blocked_getter=lambda: self._vector_persist_blocked_until_rebuild,
            vector_persist_blocked_setter=lambda v: setattr(self, '_vector_persist_blocked_until_rebuild', v),
            vector_rebuild_source_dimension_getter=lambda: self._vector_rebuild_source_dimension,
            vector_rebuild_source_dimension_setter=lambda v: setattr(self, '_vector_rebuild_source_dimension', v),
            vector_rebuild_lock_getter=lambda: self._vector_rebuild_lock,
            runtime_bundle_setter=lambda v: setattr(self, '_runtime_bundle', v),
            retriever_setter=lambda v: setattr(self, 'retriever', v),
            threshold_filter_setter=lambda v: setattr(self, 'threshold_filter', v),
            sparse_index_setter=lambda v: setattr(self, 'sparse_index', v),
            paragraph_vector_store_setter=lambda v: setattr(self, 'paragraph_vector_store', v),
            graph_vector_store_setter=lambda v: setattr(self, 'graph_vector_store', v),
            vector_store_setter=lambda v: setattr(self, 'vector_store', v),
            sparse_index_getter=lambda: self.sparse_index,
        )

        from .services.dual_vector_migration import DualVectorMigrationService
        self._dual_vector_migration_service = DualVectorMigrationService(
            vector_pool_manager=self._vector_pool_manager,
            background_scheduler=self._background_scheduler,
            cfg=self._cfg,
            active_row_filter_sql=self._active_row_filter_sql,
            dual_vector_pools_enabled=self._dual_vector_pools_enabled,
            set_embedding_degraded=self._set_embedding_degraded,
            copy_or_encode_dual_rebuild_vectors=self._vector_rebuild_service._copy_or_encode_dual_rebuild_vectors,
            graph_vector_id=self._graph_vector_id,
            rebuild_all_vectors=self._rebuild_all_vectors,
            vector_rebuild_lock_getter=lambda: self._vector_rebuild_lock,
            sleep_background=self._sleep_background,
            metadata_store_getter=lambda: self.metadata_store,
            vector_store_getter=lambda: self.vector_store,
            paragraph_vector_store_getter=lambda: self.paragraph_vector_store,
            graph_vector_store_getter=lambda: self.graph_vector_store,
