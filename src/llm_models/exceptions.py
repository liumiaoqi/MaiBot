...
class LLMTaskTimeoutError(ModelAttemptFailed):
    ...
    继承 ModelAttemptFailed 以复用调度器的“切到下一个模型”链路：当任务级硬超时被触发，
    `asyncio.wait_for` 会取消正在进行中的模型尝试，外层 `_execute_request` 会接住此异常
    并继续尝试任务 model_list 中的其它模型。若所有模型都触发超时，最终上抛给调用方。
    """
