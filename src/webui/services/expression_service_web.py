"""表达方式管理 Service 层（WebUI 专用）

从 routers/expression.py 下沉的 ORM 操作函数 + Pydantic 响应模型。
...
def get_chat_name(chat_id: str, db_session: Optional[Any] = None) -> str:
    ...
    try:
        ...
        logger.warning("操作异常 in expression_service_web.py", exc_info=True)
        return chat_id
...
# ── ORM 操作函数（从路由端点提取，接收 session 参数）──────────


def get_chat_list_data(session: Any, include_legacy: bool) -> List[ChatInfo]:
    """获取所有聊天列表数据。"""
...
def get_chat_targets_data(session: Any, include_legacy: bool) -> List[ChatInfo]:
    """获取可作为导入目标的全部已知聊天流数据。"""
...
def get_expression_groups_data(session: Any, include_legacy: bool) -> List[ExpressionGroupInfo]:
    """获取已解析的表达共享组数据。"""
...
def get_expression_list_filter_ids(
    session: Any, include_legacy: bool
) -> tuple[set[str], set[str]]:
    """获取表达方式列表的过滤 ID 集合。

    Returns:
        (visible_chat_ids, valid_expression_chat_ids)
    """
    if include_legacy:
        valid_expression_chat_ids = get_expression_session_ids(session)
        return set(), valid_expression_chat_ids
    visible_chat_ids = get_visible_expression_chat_ids(session, include_legacy=False)
    return visible_chat_ids, set()


def get_expression_list_data(
    session: Any,
    page: int,
    page_size: int,
    search: Optional[str],
    ...
    valid_expression_chat_ids: set[str],
) -> tuple[List[ExpressionResponse], int]:
    """获取表达方式列表数据。
...
def export_expressions_data(
    session: Any, chat_id: str, ids: Optional[List[int]]
) -> tuple[List[ExpressionExportItem], str]:
    """导出表达方式数据。
    ...
    if ids:
        statement = statement.where(col(Expression.id).in_(ids))
    statement = statement.order_by(
        case((col(Expression.last_active_time).is_(None), 1), else_=0),
        col(Expression.last_active_time).desc(),
    )
    ...
    expressions = session.exec(statement).all()
    if ids and len(expressions) != len(set(ids)):
    ...
    return items, chat_name


def import_expressions_data(
    session: Any, chat_id: str, expressions: List[ExpressionExportItem]
) -> tuple[int, int, int]:
    """导入表达方式数据。
...
def clear_expressions_data(session: Any, chat_id: str) -> int:
    """清除指定聊天流下的全部表达方式。
...
def get_expression_detail_data(session: Any, expression_id: int) -> Optional[ExpressionResponse]:
    """获取表达方式详细信息。

    Returns:
        ExpressionResponse 或 None（未找到时）。
    """
    statement = select(Expression).where(col(Expression.id) == expression_id).limit(1)
    expression = session.exec(statement).first()
    if not expression:
        return None
    return expression_to_response(expression, session)


def create_expression_data(
    session: Any, situation: str, style: str, chat_id: str, current_time: datetime
) -> tuple[int, ExpressionResponse]:
    """创建表达方式。
    ...
    return expression_id, data


def update_expression_data(
    session: Any, expression_id: int, update_data: Dict[str, Any]
) -> Optional[ExpressionResponse]:
    """增量更新表达方式。

    Returns:
        更新后的 ExpressionResponse 或 None（未找到时）。
    """
    ...
    return expression_to_response(db_expression, session)


def update_expression_review_status_data(
    session: Any, expression_id: int, approved: bool, current_time: datetime
) -> Optional[ExpressionResponse]:
    """切换表达方式的人工审核状态。
    ...
    db_expression.checked = approved
    db_expression.modified_by = ModifiedBy.USER if approved else None
    db_expression.last_active_time = current_time
    session.add(db_expression)
    return expression_to_response(db_expression, session)


def delete_expression_data(session: Any, expression_id: int) -> Optional[str]:
    """删除表达方式。

    Returns:
        被删除表达方式的 situation，或 None（未找到时）。
    """
    statement = select(Expression).where(col(Expression.id) == expression_id).limit(1)
    expression = session.exec(statement).first()
    if not expression:
...
def find_expression_ids(session: Any, ids: List[int]) -> List[int]:
    """查找存在的表达方式 ID。"""
    statements = select(Expression.id).where(col(Expression.id).in_(ids))
    return list(session.exec(statements).all())


def delete_expressions_by_ids(session: Any, ids: List[int]) -> int:
    """按 ID 批量删除表达方式。

    Returns:
        deleted_count
    """
    result = session.exec(delete(Expression).where(col(Expression.id).in_(ids)))
    return result.rowcount or 0


def get_expression_stats_data(session: Any, include_legacy: bool) -> Dict[str, Any]:
    """获取表达方式统计数据。
...
def get_review_stats_data(session: Any) -> ReviewStatsResponse:
    """获取审核统计数据。"""
...
def get_expression_review_logs_data(
    session: Any, limit: int, passed: Optional[bool], chat_id: Optional[str]
) -> List[ExpressionReviewLogResponse]:
    """查看最近的表达方式 AI 审核记录。"""
    ...
    return [review_log_to_response(entry, session) for entry in log_entries]


def approve_expression_review_log_data(
    session: Any,
...
) -> tuple[bool, Optional[int], ExpressionResponse]:
    """将 AI 审核日志中的表达方式设为人工审核通过，必要时从日志恢复记录。

    Returns:
        (created, restored_expression_id, data)
    """
    ...
    return created, restored_expression_id, data


def check_expression_exists(session: Any, expression_id: int) -> bool:
    """检查表达方式是否存在。"""
    expression = session.exec(select(Expression).where(col(Expression.id) == expression_id).limit(1)).first()
    return expression is not None


def review_single_expression(session: Any, expression_id: int, approved: bool) -> Optional[bool]:
    """审核单个表达方式。

    Returns:
        None 表示未找到，True 表示操作成功。
    """
    db_expression = session.exec(select(Expression).where(col(Expression.id) == expression_id).limit(1)).first()
    if not db_expression:
        return None
...
def import_legacy_expressions_data(
    session: Any,
    expression_rows: List[sqlite3.Row],
    ...
    Returns:
        (imported_count, skipped_count, failed_count, ignored_group_count)
    """
    imported_count = 0
    skipped_count = 0
    failed_count = 0
    ...
    return imported_count, skipped_count, failed_count, len(ignored_old_chat_ids)