"""黑话（俚语）管理 Service 层（WebUI 专用）

从 routers/jargon.py 下沉的 ORM 操作函数 + Pydantic 响应模型 + 辅助函数。
...
def get_latest_message_display_name(session_id: str, session: Any) -> Optional[str]:
    """读取指定聊天流的最新消息名称。"""
...
def build_session_display_name_cache(session_ids: List[str], session: Any, include_message_fallback: bool = False) -> Dict[str, str]:
...
def get_display_name_for_session_id(session_id_str: str, session: Any) -> str:
    """获取聊天流的显示名称。
...
def count_jargon_query(session: Any, statement: Any) -> int:
    """统计查询结果数量。"""
...
def get_jargon_list(
    ...
    Returns:
        (data, total): data 为黑话字典列表，total 为总条数。
    """
...
def get_chat_list(session: Any, include_empty: bool) -> List[ChatInfoResponse]:
    """获取可用于黑话新增、编辑和筛选的聊天流列表。

    Returns:
        List[ChatInfoResponse]: 已知真实聊天流，以及旧黑话记录中保留的聊天流。
    """
    seen_session_ids: Set[str] = set()
...
def get_jargon_stats(session: Any) -> Dict[str, Any]:
    """获取黑话统计数据。

    Returns:
        Dict[str, Any]: 黑话总数、确认状态和聊天分布统计。
    """
...
def get_jargon_detail(session: Any, jargon_id: int) -> Optional[JargonResponse]:
    """获取黑话详情。

    Returns:
        JargonResponse 或 None（未找到时）。
    """
    row = session.execute(
    ...
    if row is None:
        return None
    return JargonResponse(**jargon_list_row_to_dict(row, session))
...
def create_jargon(
    ...
    """创建手动黑话，覆盖同作用域的 AI 黑话。

    Args:
        session: 数据库会话。
        content: 黑话内容。
        meaning: 黑话含义。
        session_ids: 已校验的聊天流 ID 列表。
        is_global: 是否为全局黑话。

    Returns:
        JargonResponse: 新创建的黑话数据。
    """
    ...
    existing = next(
        (
            ...
            and scopes_overlap(jargon, target_session_ids, is_global)
        ),
        None,
    )
    ...
    for existing_jargon in same_content_jargons:
        if normalize_jargon_created_by(existing_jargon.created_by, existing_jargon.id) != JargonCreatedBy.AI:
            continue
        if not scopes_overlap(existing_jargon, target_session_ids, is_global):
            continue
    ...
    return JargonResponse(**jargon_to_dict(jargon, session))


def update_jargon(session: Any, jargon_id: int, update_data: Dict[str, Any]) -> Optional[JargonResponse]:
    """增量更新黑话。
    ...
    Returns:
        更新后的 JargonResponse 或 None（未找到时）。
    """
...
def delete_jargon(session: Any, jargon_id: int) -> bool:
    """删除黑话。

    Returns:
        bool: 删除成功返回 True，未找到时返回 False。
    """
    jargon = session.exec(select(Jargon).where(col(Jargon.id) == jargon_id)).first()
    if not jargon:
        return False
...
def batch_delete_jargons(session: Any, ids: List[int]) -> int:
    """批量删除黑话。

    Returns:
        int: 实际删除的记录数。
    """
    result = session.exec(delete(Jargon).where(col(Jargon.id).in_(ids)))
...
def batch_set_jargon_status(session: Any, ids: List[int], is_jargon: bool) -> int:
    """批量设置黑话状态。
