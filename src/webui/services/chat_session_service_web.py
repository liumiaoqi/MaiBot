"""聊天流管理 Service 层（WebUI 专用）

从 routers/chat/routes.py 下沉的 ORM 操作函数 + Pydantic 响应模型 + 纯转换函数。
...
def _get_latest_messages_by_session(session: Any, session_ids: List[str]) -> Dict[str, Any]:
    """批量获取每个聊天流的最新消息。"""
...
def _get_message_counts_by_session(session: Any, session_ids: List[str]) -> Dict[str, int]:
    """批量统计每个聊天流的消息数量。"""
...
def _get_expression_counts_by_session(session: Any, session_ids: List[str]) -> Dict[str, int]:
    """批量统计每个聊天流的表达数量。"""
...
def _get_jargon_counts_by_session(session: Any, session_ids: List[str]) -> Dict[str, int]:
    """批量统计每个聊天流关联的黑话数量。"""
...
def resolve_chat_targets(session: Any, targets: List[ChatTargetResolveItem]) -> List[Dict[str, Any]]:
    """批量解析配置目标到真实聊天流。"""
    ...
    latest_messages = _get_latest_messages_by_session(session, fallback_session_ids)
...
# ── 配置写入函数（async，不涉及 ORM）──────────────────────────────

async def save_chat_talk_frequency_rule(
    chat_session: ChatSession,
...
async def delete_chat_talk_frequency_rule(chat_session: ChatSession, rule_time: Optional[str]) -> None:
...
async def save_chat_learning_rule(chat_session: ChatSession, kind: str, request: LearningUpdateRequest) -> None:
...
async def save_chat_prompt_rule(
    chat_session: ChatSession,
...
async def delete_chat_prompt_rule(chat_session: ChatSession, prompt_index: int) -> None:
...
def chat_session_detail_to_response(chat_session: ChatSession) -> Dict[str, Any]:
    """构建单个聊天流的详情响应。"""
...
def release_deleted_chat_runtime(session_id: str) -> None:
...
def delete_chat_session_scope(session: Any, session_id: str) -> tuple[Dict[str, int], List[Dict[str, Any]], int]:
    """删除聊天流及所有直接归属该 session_id 的数据库记录。

    Returns:
        (jargon_result, items, total_deleted)
    """
    ...
    return jargon_result, items, total_deleted


# ── ORM 操作函数（列表/详情查询）──────────────────────────────────

def get_available_platforms(session: Any) -> List[Dict[str, Any]]:
    """获取可用平台列表。"""
    statement = (
    ...
    return [{"platform": platform, "count": count} for platform, count in platforms if platform]
...
def get_chat_sessions(session: Any, limit: int, agent_id: Optional[str]) -> List[Dict[str, Any]]:
    """获取已存在的聊天流列表。"""
    statement = (
        select(ChatSession)
        .order_by(
    ...
    latest_messages = _get_latest_messages_by_session(session, display_fallback_session_ids)
    message_counts = _get_message_counts_by_session(session, session_ids)
    expression_counts = _get_expression_counts_by_session(session, session_ids)
    jargon_counts = _get_jargon_counts_by_session(session, session_ids)
...
def find_chat_session(session: Any, session_id: str) -> Optional[ChatSession]:
    """按 session_id 查询聊天流。"""
    return session.exec(
        select(ChatSession).where(col(ChatSession.session_id) == session_id)
    ).first()