    ...
    *,
    ...
    if has_reply or has_media:
        try:
            ...
            if port is not None:
                port.report(ErrorLevel.WARNING, "构造监控消息正文失败", exception=exc)
            logger.warning(
    ...
    try:
        ...
        port = get_error_escalation_port()
        if port is not None:
            port.report(ErrorLevel.WARNING, "构造监控消息可见文本失败", exception=exc)
        logger.warning(
...
def build_monitor_message_media(message: SessionMessage, *, log_prefix: str = "") -> list[dict[str, Any]]:
    ...
    try:
        ...
        port = get_error_escalation_port()
        if port is not None:
            port.report(ErrorLevel.WARNING, "解析监控媒体默认展示模式失败", exception=exc)
