#!/usr/bin/env python3
"""exp6v5: 真正实现相对基准奖励——跑赢'买入持有'才算好

问题:v4 奖励='资产比上一步多/少'——大盘涨时资产自然多,AI 误以为'自己做对了'
真正基准奖励:
  reward = +1  if 我的资产 > 同期买入持有基准
  reward = -1  if 我的资产 < 同期买入持有基准
  → AI 学的是'跑赢大盘'(超额收益),不是'资产变多'(可能是大盘涨的)

实现:每个评估周期(60天)算一次:
  我的资产(交易后) vs 基准(期初资金按买入持有到期末)
  跑赢+1 / 跑输-1 → 调制'这个周期内的动作'
"""

import random
import sys
import os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from data_loader import POOL, load_stock, split_train_test

COST = 0.001
EVAL = 60  # 评估周期(天)

class TraderBench:
    """基准奖励 R-STDP:跑赢买入持有 = +1"""
    def __init__(self, seed=42, lr=0.05):
        random.seed(seed)
        self.lr = lr
        self.decay = 0.9
        self.w = [[random.uniform(-1.0, 1.0) for _ in range(3)] for _ in range(2)]
        self.trace_pre = [0.0, 0.0]
        self.initial = 100.0

    def step(self, rets, price, holding, cash, shares, bench_value, explore=True):
        up = max(0.0, rets[-1] * 100) if len(rets) else 0.0
        down = max(0.0, -rets[-1] * 100) if len(rets) else 0.0
        sense_spikes = [1.0 if up > 0.5 else 0.0, 1.0 if down > 0.5 else 0.0]
        noise = random.uniform(-0.2, 0.2) if explore else 0.0
        a_b = self.w[0][0]*up + self.w[1][0]*down + noise
        a_s = self.w[0][1]*up + self.w[1][1]*down + noise
        a_h = self.w[0][2]*up + self.w[1][2]*down + noise
        action = 'hold'
        if a_b > 0.1 and a_b > a_s and a_b > a_h and not holding: action = 'buy'
        elif a_s > 0.1 and a_s > a_b and a_s > a_h and holding: action = 'sell'
        if action == 'buy' and cash > 0:
            shares = cash / price * (1 - COST); cash = 0.0; holding = True
        elif action == 'sell' and holding:
            cash = shares * price * (1 - COST); shares = 0.0; holding = False
        value = cash + shares * price
        # 基准奖励:跑赢基准+1,跑输-1(只在交易时或周期末评估)
        reward = 0.0
        if action in ('buy', 'sell'):
            reward = 1.0 if value >= bench_value else -1.0
        if reward != 0.0:
            act_idx = {'buy':0, 'sell':1, 'hold':2}[action]
            post = [0.0,0.0,0.0]; post[act_idx] = 1.0
            for i in range(2):
                self.trace_pre[i] = self.trace_pre[i]*self.decay + sense_spikes[i]
                for j in range(3):
                    self.w[i][j] += self.lr * reward * self.trace_pre[i] * post[j]
        return action, holding, cash, shares

def run_bench(code, initial=100.0, seed=42):
    df = load_stock(code)
    train, test = split_train_test(df)
    trader = TraderBench(seed=seed)
    # 训练段:基准 = 期初资金按买入持有
    prices = train['close'].values
    start_p = prices[0]
    rets = []; h = False; c = initial; s = 0.0; prev = None
    for p in prices:
        if prev is not None:
            rets.append(p/prev-1); rets = rets[-20:] if len(rets)>20 else rets
            bench = initial * p / start_p  # 同期买入持有基准
            _, h, c, s = trader.step(rets, p, h, c, s, bench, explore=True)
        prev = p
    # 测试段(继续探索——真实场景 AI 也会继续学)
    tp = test['close'].values
    start_t = tp[0]
    rets3 = []; h3 = False; c3 = initial; s3 = 0.0; prev3 = None
    for p in tp:
        if prev3 is not None:
            rets3.append(p/prev3-1); rets3 = rets3[-20:] if len(rets3)>20 else rets3
            bench = initial * p / start_t
            _, h3, c3, s3 = trader.step(rets3, p, h3, c3, s3, bench, explore=True)
        prev3 = p
    return c3 + s3*tp[-1]

if __name__ == '__main__':
    print('=== exp6v5: 真基准奖励(跑赢买入持有=+1) ===')
    print('%-8s %12s %12s %12s' % ('股票', 'AI终值', '满仓基准', '超额'))
    for code, name in POOL.items():
        final = run_bench(code)
        df = load_stock(code)
        train, test = split_train_test(df)
        full = 100 * test['close'].iloc[-1]/test['close'].iloc[0]
        print('%-8s %12.1f %12.1f %+12.1f' % (name, final, full, final-full))