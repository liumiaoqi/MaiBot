#!/usr/bin/env python3
"""exp7v2: 2输入 R-STDP(单日收益 + 20日趋势方向)——趋势用符号信号"""
...
class TraderTrend2:
    ...
    def step(self, rets, price, prices_hist, holding, cash, shares, bench_value, explore=True):
        ...
        # 输入2:20日趋势方向(用价格对比:现在 vs 20天前,符号信号)
        trend = 0.0
        ...
        a_b = sum(self.w[i][0]*feats[i] for i in range(2)) + noise
        a_s = sum(self.w[i][1]*feats[i] for i in range(2)) + noise
        a_h = sum(self.w[i][2]*feats[i] for i in range(2)) + noise
        action = 'hold'
...
if __name__ == '__main__':
    print('=== exp7v2: 2输入(涨跌+20日趋势方向) ===')
    print('%-8s %12s %12s %12s' % ('股票', '趋势v2', 'v5基准', '满仓'))
