#!/usr/bin/env python3
"""exp8: 先验注入 R-STDP——人的领域知识(贝塔)作为额外输入

假设:纯价格数据是弱式有效(AI 学不到未来);
但人的先验('新能源是趋势/地产是逆风')是数据外的信息——注入后应提升。

实现:在 R-STDP 输入加一个'领域提示'特征(1=看好/0=中性/-1=看空),
模拟'懂行的人'在测试段给出的方向判断。
"""
...
# 先验提示(模拟'懂行的人'在测试段的方向判断)
# 用简单的'事后才知道的趋势方向'作为先验(理想化——验证'先验有用'这个原理)
# 茅台/长电/沪深300 = 长期上涨(看好=+1) 华夏/中石油 = 长期下跌/震荡(看空=-1)
PRIOR = {'600519': 1, '600900': 1, '601919': 0, '600340': -1,
         '000001': 0, '601857': -1, '600028': 0, '000300': 1}

class TraderPrior:
    """3输入(涨跌 + 20日趋势 + 先验提示)-> 3动作"""
    ...
    def step(self, rets, price, prices_hist, prior, holding, cash, shares, bench, explore=True):
        ret1 = rets[-1] * 100 if len(rets) else 0.0
        trend = 0.0
        if len(prices_hist) >= 21:
            trend = (price - prices_hist[-21]) / prices_hist[-21] * 100
        feats = [ret1, trend, prior * 3.0]  # 先验放大,让它有影响力
...
def run(code, initial=100.0, seed=42):
    ...
    t = TraderPrior(seed=seed)
    prior = PRIOR.get(code, 0)
    ...
    for p in prices:
        if prev is not None:
            ...
            _, h, c, s = t.step(rets, p, hist, prior, h, c, s, initial*p/start_p, explore=True)
        else: hist.append(p)
        prev = p
    ...
    for p in tp:
        if prev3 is not None:
            ...
            _, h3, c3, s3 = t.step(rets3, p, hist3, prior, h3, c3, s3, initial*p/start_t, explore=True)
        else: hist3.append(p)
        prev3 = p
    return c3 + s3*tp[-1]

if __name__ == '__main__':
    print('=== 实验8: 先验注入 R-STDP(领域知识作为额外输入) ===')
    print('%-8s %12s %12s %12s' % ('股票', '先验AI', '纯数据AI(v5)', '满仓'))
