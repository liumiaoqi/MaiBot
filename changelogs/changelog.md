...
- 插件市场安装中状态更精确，安装任务进行时会避免同时触发其他插件安装或误操作。
- 行为表现学习页新增“一句话描述聊天场景”的检索调试方式，可由 LLM 生成场景画像与 tag 簇，再查看匹配到的场景簇和行为候选。
- 行为表现学习页筛选栏与调试结果展示优化，减少无效筛选项，提升窄屏和多条件筛选时的可读性。
- 配置页、模型厂商列表、插件配置页、首页版本卡片、表达学习页和黑话管理页继续优化布局与交互，减少按钮文字挤压、统计区过宽和列表状态噪声。
- 插件配置页新增“设置 / 详情”分区，插件详情中可以查看 manifest 信息，并直达主页、仓库、文档和问题反馈链接。
- 推理过程嵌入模式下的工具栏布局优化，浏览控制与刷新按钮在嵌入场景中更稳定。
- 修复未来复古主题下插件列表竖向开关偏移的问题。

### Maisaka / 行为学习

- 裁切历史触发的表达、行为、黑话和高频词学习改为后台执行，减少长对话上下文裁切时对当前聊天流程的阻塞。
- 同一聊天流的裁切历史学习会避免重复并发批次，并在运行时停止时正确取消后台任务。
- 行为表现候选加载改为在线程中执行，减少行为检索对异步聊天主循环的阻塞。
- 行为模式维护合并时会批量读取场景簇分布，降低维护任务中的重复数据库查询。
...
- WebUI 请求错误更容易定位，接口误返回前端页面、非 JSON 响应、401 认证状态等情况会暴露更明确的错误信息。
- 聊天页、首页状态、人物管理、本地缓存图片预览等页面迁移到统一请求层后，认证、错误提示和重试行为更一致。
...
- 新增 `dashboard/CONTEXT.md`，记录 WebUI 请求层、服务端状态、配置与设置等领域词汇，方便后续重构时统一命名。

### 插件 Runtime / SDK

- Manifest 校验新增同主版本小版本兼容模式，并对兼容加载的插件输出去重警告。
- 主程序版本探测逻辑抽到 `src.plugin_runtime.detect_host_application_version`，供 RPC Server、Supervisor 和 Manifest Validator 复用。
- 插件安装进度模型增加镜像源和重试字段，后端进度广播、Git 镜像服务和前端类型定义同步更新。

### 工程与文档

- 新增轻量仓库动态 SVG 生成脚本，并调整 GitHub Actions metrics 工作流自动生成 `depends-data/repository-metrics.svg`。
- README 的仓库状态图改为引用本仓库生成的 `depends-data/repository-metrics.svg`。
