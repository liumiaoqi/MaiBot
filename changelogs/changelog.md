...
- 首页统计概览合并为单张卡片，时间范围切换移动到统计概览右上角；模型统计、日统计与活动列表改为按当前时间范围完整聚合，模型饼图改为展示花费分布，避免 30 天视图仍显示近 200 条调用或固定 7 天的数据。
- 修复外观设置中切换到原版 Dashboard 后刷新页面又回到未来复古的问题。
- 修复未来复古模式下模型管理等页面的搜索下拉菜单背景透明、内容与弹窗表单重叠显示的问题。

## 开发侧 / 修复 / SDK 与 API 改动

### 遥测与统计
- 遥测新增聚合统计上传：按平台、私聊/群聊、消息方向与回复频率区间统计消息数量，并按任务与模型别名聚合 token 使用量；本地消息记录新增 `reply_frequency`，LLM 使用记录新增 `task_name`。
- 遥测聚合统计新增统计窗口内的实际在线时长与在线覆盖率，便于区分裸时间窗口和服务实际运行时间。
